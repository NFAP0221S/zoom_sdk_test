...
const VideoFooter = (props: VideoFooterProps) => {
  ...
  const onCommandMessage = useCallback(
    async (payload: CommandChannelMsg) => {
      ...
      switch (text) {
        ...
        case 'allmute':
          {
            console.log('get user ===>', zmClient.getAllUser());
          }
          break;
        case 'sharescreenlock':
          {
            mediaStream?.lockShare(true);
            console.log('lock !!!');
          }
          break;
        case 'sharescreenunlock':
          {
  ...
  useEffect(() => {
    console.log('useEffect isStartedVideo', isStartedVideo);
  }, [isStartedVideo]);
  ...
  const onHostAskToUnmute = useCallback(
    (payload) => {
      ...
      console.log('cmdClientcmdClientcmdClient', cmdClient);
      message.warn('host님께서 마이크 on을 요청했습니다!');
